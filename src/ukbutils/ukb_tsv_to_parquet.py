...
TODO: 
    ...
    2) Allow specifying column names as input and convert only those columns to parquet.
    3) Improve the README created in the convertion process so the config part will be more readable

Usage:
    python ukb_tsv_to_parquet.py <ukb_file> <out_dir> <data_dict> [-e ENCODING] [-n NROWS] [-r REPARTITION] [-m MAX_CATEGORIES] [-d DTYPE_DICT ...] [-s SETTINGS ...] [-t TAB_OFFSET] [-f] [--log-dir LOGDIR] [--log-file-name LOGFILE] [--log-level LOGLEVEL]
...
Options:
    ...
    -dt, --dtype_dict DTYPE_DICT_TYPE: Specify dtype "Type" options in the format 'key=value'. Multiple options can be specified, separated by whitespace.
    -de, --dtype_dict_encoding DTYPE_DICT_ENCODING: Specify dtype "Encoding_type" options in the format 'key=value'. Multiple options can be specified, separated by whitespace.
    ...
    --log-dir LOGDIR: Directory to store the logfile (default: current working directory/logs/python/ukb_tsv_to_parquet).
...
def parse_args():
    ...
    arg_parser.add_argument(
        ...
        dest = "logdir",
        default = log_dir,
        help = "Directory to store the logfile. The default value is the current working directory /logs/python/ukb_tsv_to_parquet/"
    )
    arg_parser.add_argument(
        "--log-file-name", 
        dest = "logfile",
        help = "Name for the log file. If not provided, the script will generate a log file name based on the input file name and the current date and time in the format 'ukb_tsv_to_parquet_{ukb_file}_{out_dir}_{date_time}.log', where {ukb_file} is the name of the input UK biobank tsv file (without extension), {out_dir} is the name of the directory path where output is stored and {date_time} is the current date and time in the format 'YYYY-MM-DD_HH:MM:SS'."
    )
    arg_parser.add_argument(
        "--log-level", 
        choices = LOG_LEVELS, 
...
def find_date_types(dtype_conversion_dict):
    ...
    Args:
        dtype_conversion_dict (dict): Dictionary mapping data types to desired data types.
...
def determine_categorical_dtype(main_table_entry, data_dict, conversion_table_encoding, categorical_max_size=256):
    """
    Determine the appropriate Pandas CategoricalDtype for a categorical column in the UK Biobank dataset.

    Args:
        main_table_entry (pandas.Series): A row from the main table of the UK Biobank data dictionary.
        data_dict (UKB_DataDict): An instance of the UKB_DataDict class containing information about the data dictionary.
        conversion_table_encoding (dict): A dictionary mapping encoding types to Pandas dtype.
        categorical_max_size (int, optional): The maximum number of levels that can be present in a categorical column. Defaults to 256.

    Returns:
        pandas.CategoricalDtype or str: The Pandas dtype for the categorical column or a special keyword if exceeding the maximum levels.

    Raises:
        ValueError: If the necessary information for determining the dtype is not found in the provided data.
    ...
    if np.isnan(num_members) or main_table_entry["Encoding_type"].isna().item():
        raise ValueError("Trying to parse {} as categorical, but no \"Encoding_num_members\" item found in given dataframe (encoding id: {}, encoding type: {}, encoding num members: {})"
                         .format(
                             main_table_entry["UDI"], 
                             main_table_entry["Encoding_id"], 
                             main_table_entry["Encoding_type"],
                             main_table_entry["Encoding_num_members"]
                         ))
    ...
    if num_members <= categorical_max_size:
        ...
        my_dtype = EXCEEDING_MAX_CAT_KEYWORD #conversion_table[main_table_entry["Encoding_type"].item()]
    
    return(my_dtype)
            

def determine_column_dtype(column_name, data_dict, conversion_table, cat_cols=[], conversion_table_encoding=None, categorical_max_size=256, type_column="Type"):
    """
    Determine the appropriate Pandas dtype for a column in the UK Biobank dataset, considering optional categorical columns.

    Args:
        column_name (str): The name of the column in the UK Biobank dataset.
        data_dict (UKB_DataDict): An instance of the UKB_DataDict class containing information about the data dictionary.
        conversion_table (dict): A dictionary mapping UK Biobank types to Pandas dtype.
        cat_cols (list, optional): A list of column types (e.g. 'Categorical (single)'), columns with this type will be treated as categorical. Defaults to an empty list.
        conversion_table_encoding (dict, optional): A dictionary mapping encoding types to Pandas dtype. Required if cat_cols is provided.
        categorical_max_size (int, optional): The maximum number of levels that can be present in a categorical column. Defaults to 256.
        type_column (str, optional): The name of the column containing the UK Biobank types. Defaults to "Type".

    Returns:
        Pandas dtype: The Pandas dtype for the specified column.

    Raises:
        ValueError: If the necessary information for determining the dtype is not found in the provided data or if an invalid configuration is detected.
    ...
    # TODO: add test to check if there is a single row for the given column name in the main table, throw error if not found or multiple rows    
    ukb_type = main_table_entry[type_column].item()

    if ukb_type in cat_cols:
        my_d_type = determine_categorical_dtype(main_table_entry, data_dict, conversion_table_encoding, categorical_max_size)
    
    else:
        try:
            my_d_type = conversion_table[ukb_type]
        except KeyError:
            raise ValueError(f"The type {ukb_type} could not be translated to a dtype, because it was not present in given conversion_table ({conversion_table})")
...
def get_date_format_dict(use_columns, data_dict, dtype_dict, max_categories):
    ...
    Args:
        ...
        max_categories (int): Maximum levels allowed in a categorical column for dtype conversion.
    ...
    for col_name in use_columns:
        ...
        if col_type in date_type_keys:
            ...
            logging.debug(f"Column '{col_name}' identified as date type with format: {dtype_dict['Type'][col_type][1]}")
    ...
    for col_name in encoding_col_columns:
        ...
        if n_categories > max_categories:
            ...
            if col_encoding_type in date_type_encoding_keys:
                date_format_dict[col_name] = dtype_dict["Encoding_type"][col_encoding_type][1]
                encoding_date_columns_found += 1
                logging.debug(f"Column '{col_name}' identified as date type with format: {dtype_dict['Encoding_type'][col_encoding_type][1]}")

    logging.info(f"Identified {date_columns_found} date columns and {encoding_date_columns_found} encoding date columns.")
...
def get_types_dict(use_columns, data_dict, dtype_dict, cat_cols, max_num_categories):
    ...
    # Call the determine_column_dtype function for every column that is not considered a "Date" type
    types_dict = {
        col_name: determine_column_dtype(col_name, data_dict, dtype_dict["Type"], cat_cols, dtype_dict["Encoding_type"], max_num_categories)
        for col_name in use_columns 
    ...
    unprocessed_non_date_cols = {
        col_name: determine_column_dtype(col_name, data_dict, dtype_dict["Encoding_type"], type_column="Encoding_type") 
        for col_name in types_dict
        if (
            types_dict[col_name] == EXCEEDING_MAX_CAT_KEYWORD
            and not col_name in unprocessed_date_cols
        )
    }
...
def get_dask_dataframe(tsv_file, col_names, offset, data_dict, dtype_dict, cat_cols, max_categories, encoding):
    """
    ...
    date_format_dict = get_date_format_dict(col_names, data_dict, dtype_dict, max_categories)
...
def convert_to_parquet(tsv_file_in, out_dir, data_dict, column_names, nrows=0, npartitions=0, dtype_dict=DEFAULT_DTYPE_DICT, settings={}, offset=0, force=False, cat_cols=DEFAULT_CATEGORICAL_COLUMNS, max_categories=DEFAULT_MAX_CATEGORIES, encoding=DEFAULT_ENCODING):
    """
...
def set_dtype_in_dtype_dict(dtype_dict, key, dtype_val):
    """
    Set the value for the specified key in the dtype_dict, overwriting the existing value if present.
    ...
    Args:
        ...
        dtype_val (str or list): Desired data type value to be associated with the key.
...
def add_dtype_arg_to_dict(dtype_arg, dtype_dict):
    ...
    if value.startswith("[") and value.endswith("]"):
        try:
            ...
            # logging.warning(f"Failed to parse value as JSON list: {value}. Skipping.")
            
        if len(arg_list) == 2 and arg_list[0].lower() in VALID_DTYPE_KEYWORDS:
            set_dtype_in_dtype_dict(dtype_dict, key, arg_list)
        else:
            raise ValueError(f"Invalid value for {key}: {value}")
            # logging.warning(f"Invalid value for {key}: {value}. Skipping.")
...
def get_required_type(data_dict, cat_cols, encoding_type=False):
    """
    Get the unique list of required data types based on the given data dictionary and categorical columns.

    Args:
        data_dict (UKB_DataDict): An instance of the UKB_DataDict class containing information about the data dictionary.
        cat_cols (list): List of categorical columns.
        encoding_type (bool, optional): If True, retrieves encoding types. If False (default), retrieves regular types.

    Returns:
        list: Unique list of required data types that need to be present in the dtype dict.
    """
    types_list = []
    if encoding_type:
...
def create_missing_type_set(present_types, required_types):
    """
    Create a set of missing data types based on the given present types and required types.
...
def get_missing_types_in_dtype_dict(dtype_dict, data_dict, cat_cols):
    """
    Get the missing data types in the dtype dictionary based on the given data dictionary and categorical columns.
    ...
    Returns:
        dict: Dictionary with two keys "Type" and "Encoding_type", for each of these the missing data types will be stored in a list.
    """
    ...
    types_not_present = {
        ...
        "Encoding_type": create_missing_type_set(dtype_dict["Encoding_type"].keys(), required_encoding_types)
    }
...
def parse_dtype_dict_args(dtype_args, default_dict):
    """
    Parse dtype arguments and create a dictionary mapping data types to desired dtype.
    ...
    if dtype_args:    
        ...
        if len(dtype_arg_set) < len(dtype_args):
            logging.warning("Duplicated dtypes found in given dtype arguments. Make sure to specify each dtype once.")
    
    logging.info(f"Using the following dtype dict to convert data types:\n{json.dumps(dtype_dict, indent=4)}")
    return dtype_dict
...
def main(args):
    ...
    dtype_dict = {}
    for dtype_dict_type in ["Type", "Encoding_type"]: #[args.dtype_dict_type, args.dtype_dict_encoding]:
        dtype_dict_args = eval(f"args.dtype_dict_{dtype_dict_type.lower()}")
        dtype_dict[dtype_dict_type] = parse_dtype_dict_args(dtype_dict_args, DEFAULT_DTYPE_DICT[dtype_dict_type])

    missing_types = get_missing_types_in_dtype_dict(dtype_dict, data_dict, args.categorical_cols)
...
def main_cli():
    """
    Entry point for the command-line interface (CLI) of the UK Biobank TSV to Parquet conversion tool.
    ...
    try:
        ...
        utils.exit_script(
            ...
            status=1, 
            exit_message=f"An exception while converting {args.ukb_file.name} to {args.out_dir} parquet file, see {logging.getLoggerClass().root.handlers[0].baseFilename}"
        )
