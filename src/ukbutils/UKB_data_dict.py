...
class UKB_DataDict():
    """
    A class for working with UK Biobank Data Dictionary HTML files generated by the 'ukbconv' utility.
    
    The 'ukbconv' utility creates an HTML document that lists information about the structure of the dataset,
    including data coding information. This class provides methods for parsing and extracting relevant
    information from the generated HTML file.

    TODO: 
        - When new UKB_DataDict is initialized check the encoding_file_template path to see if there are encoding files, if so add to current encodings in cache. (If too many exceeding encoding_table_limit, what to do?)
        - Some way to save state of UKB_DataDict object?
    """

    ENCODING_DOWNLOAD_URL_TEMPLATE = "https://biobank.ndph.ox.ac.uk/showcase/codown.cgi?id={}"
    DEFAULT_ENCODING_FILE_TEMPLATE = "~/ukb-analyses/data/ukb/encoding_dicts/encoding_table_{}.txt"
    
    def __init__(self, path, encoding_file_template=DEFAULT_ENCODING_FILE_TEMPLATE, encoding_table_limit=100):
        """
        Initialize the UKB_DataDict object.

        Args:
            path (str): The path to the UK Biobank Data Dictionary HTML file.
            encoding_file_template (str, optional): The template for encoding table file paths.
            encoding_table_limit (int, optional): The limit for the number of encoding tables to be stored in the cache.
        """
    ...
    def path_to_html(self, path):
        ...
        if path != self._path_to_html:
            if os.access(path, os.R_OK) and path.endswith(".html"):
                ...
                raise ValueError("Path must be an valid HTML file with read access. {} could not be read or was not an HTML file.".format(path))
    ...
    def main_table(self):
        """
        pd.DataFrame: The main table from the HTML data dictionary that lists information about the structure of the dataset.
        """
    ...
    def info(self):
        """
        dict: Metadata scraped from the top of the HTML data dictionary file. It contains information
        about the extraction date and the number of data columns in the dataset.
        """
    ...
    def encoding_table_limit(self):
        """
        int: The limit for the number of encoding tables to be stored in the cache.
        """
    ...
    def encoding_table_limit(self, limit):
        ...
        Args:
            limit (int): The new limit on the number of encoding tables to be stored in the cache.
    ...
    def _process_main_table(self, main_table):
        """
        Process the main data dictionary table, adding derived columns, and setting appropriate dtypes.
        ...
        main_table["Encoding_id"] = main_table["Description"].str.extract(r"Uses data-coding (\d+)")
        main_table["Encoding_num_members"] = main_table["Description"].str.extract(r"Uses data-coding \d+ comprises (\d+)").astype("Int64")
        main_table["Encoding_type"] = main_table["Description"].str.extract(r"Uses data-coding \d+ comprises \d+ (\w+)-valued")
        main_table["Is_hierarchical"] = main_table["Description"].str.contains("hierarchical")
    ...
    def _download_encoding_table(self, encoding_id):
        """
        Download an encoding table from the UK Biobank website using the curl command inside a subprocess.
    
        Args:
            encoding_id (int): The ID of the encoding table to download.
    
        Raises:
            ValueError: If the provided encoding ID is not valid (does not exist in the main table).
        """
        ...
        command_list = ["curl", url, "-silent", "--create-dirs", "-o", encoding_dest_filename]
        sp.run(command_list, stdout=open(os.devnull, 'wb'))

    
    def _get_encoding_table_from_file(self, encoding_id):
        """
        Get an encoding table from a local file, if not present try to download it with the _download_encoding_table method.
        ...
        Raises:
            ValueError: If the file is inaccessible or if the UK Biobank website returns an error.
        """
        ...
        if not os.path.isfile(encoding_filename):
            ...
            raise ValueError("File with encoding table {} does not have read access.".format(encoding_filename))
        ...
        if "Sorry, internal error prevents download of coding" in file_content:
            ...
            raise ValueError("UK Biobank website returned an internal server error while trying to download encoding table for encoding id {}".format(encoding_id))
    ...
    def _get_encoding_table_from_html(self, encoding_id):
        ...
        """
        encoding_table_html = self.html_file_content.find('table', summary="Coding {}".format(encoding_id))

        if not encoding_table_html:
            raise ValueError("No encoding table found for encoding with encoding ID: {}".format(encoding_id))
    ...
    def _encoding_is_hierarchical(self, encoding_id):
        """
        Check if an encoding is hierarchical, with the main_table Is_hierarchical column.
        ...
        """
        return self.main_table[self.main_table["Encoding_id"] == encoding_id].iloc[0]["Is_hierarchical"]

    
    def _retrieve_encoding_table(self, encoding_id):
        """
        Retrieve an encoding table, either from a file (if hierarchical) or from HTML (if not hierarchical).
        ...
        Raises:
            ValueError: If the encoding ID is not found or an error occurs during retrieval.
        """
        if not self._encoding_id_is_valid(encoding_id):
            raise ValueError("Encoding ID ({}) not found in main table.".format(encoding_id))
    ...
    def _calc_encoding_file_size(self):
        ...
        """
        total_size = sum(os.path.getsize(self.encoding_filename.format(enc_id)) / (1024 * 1024)
    ...
    def _remove_oldest_if_needed_encoding_cache(self):
        """
        Remove the oldest encoding tables from the cache if it exceeds the specified limit (property).
        """
    ...
    def _add_encoding_table_to_cache(self, encoding_id):
        ...
        """
        self._data_encoding_tables[encoding_id] = self._retrieve_encoding_table(encoding_id)
        self._saved_encodings.append(encoding_id)
        self._remove_oldest_if_needed_encoding_cache()
    ...
    def get_encoding_table(self, encoding_id):
        ...
        Raises:
            ValueError: If the encoding table cannot be read or an error occurs during retrieval.
        """
        if not encoding_id in self._data_encoding_tables.keys():
            try:
                self._add_encoding_table_to_cache(encoding_id)
            except Exception as e:
                raise ValueError("No encoding table could be read for encoding id {}.".format(encoding_id)) from e
        else:
    ...
    def get_encoding_by_data_field(self, field_id, is_udi=True):
        """
        Get an encoding table associated with a specific data field ID, either by UDI (unique data identifier) or by the first part of the UDI.
    
        Args:
            field_id (str): The data field ID (either a complete UDI or its first part).
            is_udi (bool, optional): If True, `field_id` is treated as a complete UDI; if False, it's treated as the first part of the UDI.
    
        Returns:
            pd.DataFrame: The retrieved or cached encoding table associated with the specified data field ID.
    
        Raises:
            ValueError: If the encoding table cannot be read or an error occurs during retrieval,
                        or if the specified data field ID is not found in the main table.
        """
        if is_udi:
            if not field_id in self.main_table["UDI"].unique():
                raise ValueError("Given field_id ({}) not found in the UDI column of the main table.".format(field_id))
            ...
            if not field_id in split_udi_id.unique():
                raise ValueError("Given field_id ({}) not found as part of a UDI in the UDI column of the main table.".format(field_id))
    ...
    def get_human_readable_name(self, field_id, is_udi=True):
        ...
        Raises:
            ValueError: If the specified field ID is not found in the main table UDI column or if it's ambiguous (multiple names found).
        """
        if is_udi:
            if not field_id in self.main_table["UDI"].unique():
                raise ValueError("Field ID ({}) not found in the UDI column of the main table. Please check the provided field ID.".format(field_id))
            ...
            split_udi_id = self.main_table["UDI"].str.split("-", expand=True)[0]
            if not field_id in split_udi_id.unique():
                raise ValueError("Field ID ({}) not found as part of a UDI in the UDI column of the main table. Please check the provided field ID.".format(field_id))
            ...
            if description_series.size == 1:
                ...
                if len(description_series.unique()) > 1:
                    raise ValueError("Ambiguous field_id: {}. Multiple non-identical human-readable names found.".format(field_id))
                else:
