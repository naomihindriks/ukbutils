...
This script converts UK Biobank TSV data to Parquet format based on a provided
configuration file. It uses the `ukb_tsv_to_parquet` module for the conversion
process.
...
Parameters:
    ...
    <html_data_dict> (str): Path to the HTML data dictionary file (created
        using the `ukbconv` program with the option `docs`).
    <config_file> (str): Path to the YAML configuration file.

Optional Arguments:
    --log-dir <logdir> (str): Directory to store the logfile. Default is
        'logs/python/create_parquet_from_config/' in the current working directory.
    --log-file-name <logfile> (str): Name for the log file. If not provided,
        a default name is generated based on input file names and the current
        date and time.
    --log-level <loglevel> (str): Set the level for the log file. Available
...
def parse_args():
    ...
    the script. It expects three required arguments: 'ukb_file', 'html_data_dict',
    and 'config_file'. Optional arguments include '--log-dir', '--log-file-name',
    and '--log-level'. The default log directory is
    'logs/python/create_parquet_from_config/' in the current working directory.
    The log file is named based on input files and the current date and time if
    not provided. The log level defaults to 'INFO' but can be set to 'DEBUG',
...
def set_logfile(args):
    ...
    If 'logfile' is not specified, a default name is generated using input
    file names and the current date and time. The log directory is created if
    it doesn't exist. Configures the logging module with the specified log file,
    level, format, and file mode.
    """
...
def find_first_existing_parent(path):
    ...
    Returns:
        str: The path of the first existing parent directory. If the path does
            not exist, the function returns the current working directory.
    """
...
def check_if_output_dir_is_ok(path, force):
    ...
    Args:
        path (str): The path to the directory.
        force (bool): The force flag indicating whether to overwrite existing
            content.
...
def get_data_dict(html_file):
    ...
    This function initializes a UKB_DataDict instance by parsing the provided
    HTML file. The UKB_DataDict object is ready to parse and extract information
    from the HTML data dictionary.
...
def convert_config_to_parquet(file_in, data_dict, config, col_names):
    missing_types = tsv2parquet.get_missing_types_in_dtype_dict(
...
def main_cli():
    # Function to run when program termination signal is recieved, will write
    # a line to logfile
