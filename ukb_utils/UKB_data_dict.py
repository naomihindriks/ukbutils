...
class UKB_DataDict():
    ...
    def info(self):
        ...
        if self._info is None:
            ...
            info_table_pd = pd.read_html(str(info_table_html))[0]
            self._info = {
    ...
    def __init__(self, path, encoding_file_template=str(Path.cwd().parent) + "/data/ukb/encoding_dicts/encoding_table_{}.txt"):
        """
        ...
        self._encoding_file_template = encoding_file_template
    ...
    def _get_main_from_html(self):
        ...
        return pd.read_html(str(html_content))[0]
    ...
    def _download_encoding_table(self, encoding_id):
        """
        Download an encoding table from the UK Biobank website.

        Args:
            encoding_id (int): The ID of the encoding table to download.
        """
        url = self.ENCODING_DOWNLOAD_URL_TEMPLATE.format(encoding_id)
        command_list = ["curl", url, "-silent", "--create-dirs", "-o", self._encoding_file_template.format(encoding_id)]
        sp.run(command_list, stdout=open(os.devnull, 'wb'))

    
    def _get_encoding_table_from_file(self, encoding_id):
        """
        Get an encoding table from a local file, if not prsent try to download it with the _download_encoding_table method.
        ...
        """
        encoding_filename = self._encoding_file_template.format(encoding_id)
    ...
    def _get_encoding_table_from_html(self, encoding_id):
        ...
        return pd.read_html(str(encoding_table_html))[0]
    ...
    def _calc_encoding_file_size(self):
        ...
        """
        total_size = sum((os.path.getsize(self._encoding_file_template.format(enc_id)) / (1024 * 1024))
                 for enc_id in self._saved_encodings
                 if os.path.exists(self._encoding_file_template.format(enc_id)))
        return(total_size)

    
    def _remove_oldest_if_needed_encoding_cache(self):
        """
        Remove the oldest encoding table from the cache if it exceeds a certain size.
        """
        if len(self._saved_encodings) > 100:
    ...
    def get_human_readable_name(self, field_id, is_udi=True):
        if is_udi:
            ...
            if description_series.size == 1:
                description = enc_id_series.item()
            else:
                ...
                if len(description_series.unique()) > 1:
                    ...
                    description = enc_id_series.iloc[0]
